...
Cantina will accept a song acquired with
[Geomitron Bridge](https://github.com/Geomitron/Bridge), add it to Barkeep's
...
The project brief targets Bridge 3.4.0. The latest published release inspected on
2026-08-01 is 3.4.5. Both are Electron GUI applications with the same relevant
integration limit:
...
- there is no documented local HTTP/WebSocket API, named pipe, CLI, custom URL scheme,
  or reusable SDK;
- a download is staged in Bridge's private data directory, then moved to its configured
  library as a chart folder or `.sng` file;
- Bridge neither requests a YARG rescan nor changes a YARG play queue.

The evidence is Bridge's
...
1. The operator installs Bridge from its official release and uses Bridge's UI to
   select a dedicated subdirectory of a YARG song source.
2. Bridge is configured to retain `.sng` files. Folder extraction is not a supported
   Cantina path until its archive and cross-volume behavior passes issue #17.
3. Barkeep is independently configured with the same canonical directory. It does not
   read or modify Bridge's private `settings.json`, `library.db`, or temp directory.
4. Bridge owns search and the act of starting/canceling a download. Barkeep observes
   only the final library directory.
...
Bridge writes to its final configured directory; Barkeep does not move the file away
and defeat Bridge's path-based duplicate check. Startup and periodic reconciliation
...
1. a versioned Bridge CLI, deep link, or authenticated loopback API accepted upstream;
...
automation, Bridge settings/database coupling, and undocumented provider download URLs
are not acceptable adapters.

## Licensing and distribution

Bridge declares GPL-3.0 and remains a separate operator-installed program. Cantina is
LGPL-3.0-or-later and does not vendor, link, modify, repackage, or silently update
Bridge. Bridge source may inform interoperability research but is not copied into this
...
Issue #17 closes only with evidence from the Windows 10 theater PC for Bridge 3.4.0 or
