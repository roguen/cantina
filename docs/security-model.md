...
Song acquisition adds an untrusted-content boundary. Before Bridge arrivals or a
future remote provider can trigger YARG or setlist changes, the design must also
resolve:
...
- reconciliation and idempotency across watcher storms, cancellation, Bridge or
  Barkeep crashes, antivirus locks, and duplicate arrivals;
...
Bridge must be installed and updated explicitly by the operator. Cantina neither
